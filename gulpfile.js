...
const PATHS_TO_STATIC = [
  `${PATH_TO_SOURCE}fonts/**/*.{woff2,woff,ttf,otf}`,
  `${PATH_TO_SOURCE}*.ico`,
...
export function processScripts () {
  ...
  const getWebpackConfig = require('./webpack.config.cjs');
  const webpackConfig = getWebpackConfig(
    {},
    { mode: isDevelopment ? 'development' : 'production' }
  );

  return src(`${PATH_TO_SOURCE}scripts/index.js`)
    .pipe(plumber())
...
export function startServer () {
  ...
  watch(`${PATH_TO_SOURCE}**/*.pug`, series(processMarkup));
